// Subcontractor/Technician chat UI for Issue doctype (technician_support channel)
// Adds a "Chat with Sub Contractor" button and contains the full chat UI.
// Uses the technician_support channel; sender_type distinguishes technician vs subcontractor per message.

function ensure_chat_with_sub_contractor_button(frm) {
  if (!frm || frm.doctype !== "Issue") return;

  const label = __("Chat with Technician");
    ...
    frm.add_custom_button(label, () => {
      if (!frm.doc.person_in_charge) {
        frappe.msgprint({
          ...
          message: __(
            "Please assign Person in Charge on this issue before using Chat with Technician."
          ),
          indicator: "orange",
        });
        return;
      }
      // Switch the shared chat UI to technician/subcontractor channel
      window.__propms_issue_chat_channel = 'technician_support';
      if (typeof load_tenant_chat_emoji_assets_once === 'function') load_tenant_chat_emoji_assets_once();
      if (typeof propms_init_tenant_chat_ui === 'function') propms_init_tenant_chat_ui();
      if (typeof openChat === 'function') openChat(frm);
    });
  } catch (e) {
    // ignore; refresh will retry
  }
}

frappe.ui.form.on('Issue', {
  onload_post_render(frm) {
    ensure_chat_with_sub_contractor_button(frm);
  },
  refresh(frm) {
    ensure_chat_with_sub_contractor_button(frm);
  },
...
try {
  frappe.after_ajax(() => {
    if (window.cur_frm && window.cur_frm.doctype === "Issue") {
      ensure_chat_with_sub_contractor_button(window.cur_frm);
    }
  });
} catch (e) {}
...
`;
    ...
    function loadMessagesFromServer(clearArea) {
      ...
      var channel = window.__propms_issue_chat_channel || 'technician_support';
      frappe.call({
