...
class SubContractorShiftAssignment(Document):
	def validate(self):
		validate_active_sub_contractor(self.sub_contractor)
		if self.end_date:
	...
	def on_cancel(self):
		self.validate_sub_contractor_checkin()
		self.validate_attendance()

	def validate_sub_contractor_checkin(self):
		checkins = frappe.get_all(
			"Sub Contractor Checkin",
			filters={
		...
		if checkins:
			frappe.throw(
				_("Cannot cancel Shift Assignment: {0} as it is linked to Sub Contractor Checkin: {1}").format(
					self.name, get_link_to_form("Sub Contractor Checkin", checkins[0])
				)
			)
		...
		if attendances:
			frappe.throw(
				_("Cannot cancel Shift Assignment: {0} as it is linked to Attendance: {1}").format(
					self.name, get_link_to_form("Sub Contractor Attendance", attendances[0])
				)
			)
			...
			if not self.docstatus:
				frappe.msgprint(
					_(
						...
						frappe.bold(self.sub_contractor),
						get_link_to_form("Sub Contractor Shift Assignment", overlapping_dates[0].name),
					)
				)
		else:
	...
	def get_overlapping_dates(self):
		if not self.name:
			self.name = "New Sub Contractor Shift Assignment"

		shift = frappe.qb.DocType("Sub Contractor Shift Assignment")
		query = (
			frappe.qb.from_(shift)
			.select(shift.name, shift.shift_type, shift.docstatus, shift.status)
			.where(
				(shift.sub_contractor == self.sub_contractor)
				& (shift.docstatus == 1)
	...
	def throw_overlap_error(self, shift_details):
		...
		if shift_details.docstatus == 1 and shift_details.status == "Active":
			msg = _(
				"Sub Contractor {0} already has an active Shift {1}: {2} that overlaps within this period."
			).format(
				frappe.bold(self.sub_contractor),
				frappe.bold(shift_details.shift_type),
				get_link_to_form("Sub Contractor Shift Assignment", shift_details.name),
			)
...
def get_events(start, end, filters=None):
	sub_contractor = frappe.db.get_value(
		"Sub Contractor", {"user_id": frappe.session.user}, ["name", "company"], as_dict=True
	)
	if sub_contractor:
		sub_contractor = sub_contractor.name
	else:
		sub_contractor = ""
...
def get_shift_assignments(start: str, end: str, filters: str | list | None = None) -> list[dict]:
	...
	return frappe.get_list(
		"Sub Contractor Shift Assignment",
		filters=filters,
	...
	for d in assignments:
		...
		while daily_event_start <= daily_event_end:
			...
			event = {
				...
				"end_date": end_timing,
				"title": cstr(d.sub_contractor_name) + ": " + cstr(d.shift_type),
...
def get_shifts_for_date(sub_contractor: str, for_timestamp: datetime) -> list[dict[str, str]]:
	...
	assignment = frappe.qb.DocType("Sub Contractor Shift Assignment")
	return (
...
def get_shift_for_timestamp(sub_contractor: str, for_timestamp: datetime) -> dict:
	shifts = get_shifts_for_date(sub_contractor, for_timestamp)
	if shifts:
		return get_shift_for_time(shifts, for_timestamp)
	return {}


def get_sub_contractor_shift(
	sub_contractor: str,
	for_timestamp: datetime | None = None,
	consider_default_shift: bool = False,
	next_shift_direction: str | None = None,
) -> dict:
	"""Returns a Shift Type for the given Sub Contractor on the given date

	:param sub_contractor: Sub Contractor for which shift is required.
	...
	shift_details = get_shift_for_timestamp(sub_contractor, for_timestamp)

	# if shift assignment is not found, consider default shift
	default_shift = frappe.db.get_value("Sub Contractor", sub_contractor, "default_shift", cache=True)
	...
	if not shift_details and next_shift_direction:
		shift_details = get_prev_or_next_shift(
			sub_contractor, for_timestamp, consider_default_shift, default_shift, next_shift_direction
		)
...
def get_prev_or_next_shift(
	...
	if consider_default_shift and default_shift:
		...
		for i in range(MAX_DAYS):
			...
			shift_details = get_sub_contractor_shift(sub_contractor, date, consider_default_shift, None)
			if shift_details:
		...
		for date_range in shift_dates:
			...
			for dt in generate_date_range(start_date, end_date, reverse=reverse):
				shift_details = get_sub_contractor_shift(
					sub_contractor, datetime.combine(dt, for_timestamp.time()), consider_default_shift, None
				)
				if shift_details:
					return shift_details
...
def get_sub_contractor_shift_timings(
	sub_contractor: str, for_timestamp: datetime | None = None, consider_default_shift: bool = False
) -> list[dict]:
	"""Returns previous shift, current/upcoming shift, next_shift for the given timestamp and sub_contractor"""
	if for_timestamp is None:
		for_timestamp = now_datetime()

	# write and verify a test case for midnight shift.
	prev_shift = curr_shift = next_shift = None
	curr_shift = get_sub_contractor_shift(sub_contractor, for_timestamp, consider_default_shift, "forward")
	if curr_shift:
		next_shift = get_sub_contractor_shift(
			sub_contractor,
			curr_shift.start_datetime + timedelta(days=1),
			consider_default_shift,
			"forward",
		)
	prev_shift = get_sub_contractor_shift(
		sub_contractor,
...
def get_actual_start_end_datetime_of_shift(
	sub_contractor: str, for_timestamp: datetime, consider_default_shift: bool = False
) -> dict:
	...
	:param sub_contractor (str): Sub Contractor name
	...
	default shift in sub_contractor master if no shift assignment is found
	"""
	shift_timings_as_per_timestamp = get_sub_contractor_shift_timings(
		sub_contractor, for_timestamp, consider_default_shift
	)
...
def validate_active_sub_contractor(sub_contractor, method=None):
	if isinstance(sub_contractor, dict | Document):
		sub_contractor = sub_contractor.get("sub_contractor")
