...
from propms.property_management_solution.doctype.sub_contractor_shift_assignment.sub_contractor_shift_assignment import get_actual_start_end_datetime_of_shift
...
class SubContractorCheckin(Document):
	...
	def validate_duplicate_log(self):
		...
		if doc:
			doc_link = frappe.get_desk_link("Sub Contractor Checkin", doc)
			frappe.throw(
				_("This sub contractor already has a log with the same timestamp.{0}").format("<Br>" + doc_link)
			)
	...
	def fetch_shift(self):
		if not (
			shift_actual_timings := get_actual_start_end_datetime_of_shift(
				self.sub_contractor, get_datetime(self.time), True
			)
		):
			self.shift = None
			self.offshift = 1
...
def bulk_fetch_shift(checkins: list[str] | str) -> None:
	...
	for d in checkins:
		doc = frappe.get_doc("Sub Contractor Checkin", d)
		doc.fetch_shift()
...
def mark_attendance_and_link_log(
	...
	"""Creates an attendance and links the attendance to the Sub Contractor Checkin.
	Note: If attendance is already present for the given date, the logs are marked as skipped and no exception is thrown.

	:param logs: The List of 'Sub Contractor Checkin'.
	:param attendance_status: Attendance status to be marked. One of: (Present, Absent, Half Day, Skip). Note: 'On Leave' is not supported by this function.
	:param attendance_date: Date of the attendance to be created.
	:param working_hours: (optional)Number of working hours for the given date.
	"""
	log_names = [x.name for x in logs]
	sub_contractor = logs[0].sub_contractor
	...
	if attendance_status == "Skip":
		...
		try:
			...
			if attendance := get_existing_half_day_attendance(sub_contractor, attendance_date):
				frappe.db.set_value(
				...
				attendance = frappe.new_doc("Sub Contractor Attendance")
				attendance.update(
					{
						"doctype": "Sub Contractor Attendance",
			...
			if attendance_status == "Absent":
				attendance.add_comment(
					text=_("Sub Contractor was marked Absent for not meeting the working hours threshold.")
				)
...
def get_existing_half_day_attendance(sub_contractor, attendance_date):
	...
	if attendance_name:
		attendance_doc = frappe.get_doc("Sub Contractor Attendance", attendance_name)
...
def calculate_working_hours(logs, check_in_out_type, working_hours_calc_type):
	...
	:param logs: The List of 'Sub Contractor Checkin'.
	:param check_in_out_type: One of: 'Alternating entries as IN and OUT during the same shift', 'Strictly based on Log Type in Sub Contractor Checkin'
		...
		frappe.get_doc(
			{
				...
				"reference_doctype": "Sub Contractor Checkin",
				"reference_name": name,
				"content": text,
			}
...
def skip_attendance_in_checkins(log_names: list):
	SubContractorCheckin = frappe.qb.DocType("Sub Contractor Checkin")
	(
		frappe.qb.update(SubContractorCheckin)
		.set("skip_auto_attendance", 1)
		.where(SubContractorCheckin.name.isin(log_names))
	).run()
...
def add_log_based_on_subcontractor_field(
    ...
    if subcontractor:
        ...
        #     _("No Sub Contractor found for the given subcontractor field value. '{}': {}").format(
        #         subcontractor_fieldname, subcontractor_field_value
        #     ),
        # )
        frappe.throw(
