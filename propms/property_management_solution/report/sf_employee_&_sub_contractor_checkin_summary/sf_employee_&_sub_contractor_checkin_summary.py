...
from frappe.utils import get_time, add_to_date, getdate
...
def get_data(filters):
	...
	if fetch_employees:
		...
		result.extend(process_checkin_records(emp_checkin_details, shift_types_map))

	if fetch_subcontractors:
		sub_checkin_details = get_sub_checkin_details(filters)
		result.extend(process_checkin_records(sub_checkin_details, shift_types_map))
...
def process_checkin_records(checkin_details, shift_types_map):
	rows = []
...
def get_emp_conditions(filters):
	conditions = " AND IFNULL(emp.sf, 0) = 1"
...
def get_sub_conditions(filters):
	conditions = " AND IFNULL(sc.sf, 0) = 1"
	if filters.get("from_datetime"):
		conditions += " AND chec.time >= %(from_datetime)s"
	elif filters.get("from_date"):
		conditions += " AND DATE(chec.time) >= %(from_date)s"
